...
public class EngineTests : IntegrationTestBase
{
	...
	{
		var initialPlacement = new EngineResponse(null, null, new Point(initialLocationX, initialLocationY), initialDirection);
		var moveEngine = Engines.FirstOrDefault(x => x.Name.IsEqualTo(nameof(MoveEngine)));

		moveEngine.ShouldNotBeNull();

		var engineResponse = moveEngine.Execute(TableDimensionConfig, initialPlacement, string.Empty);

		var reportEngineResponse = ReportEngine.Execute(TableDimensionConfig, engineResponse, string.Empty);
